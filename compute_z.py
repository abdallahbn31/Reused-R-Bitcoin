...
Notes:
 - Supports Legacy (SIGHASH_ALL) and SegWit v0 (BIP143) SIGHASH_ALL only.
...
def compute_z_for_row(row, fetch_prevouts=False):
    ...
    # If tx is segwit (or parsed as segwit) and we have prev_value and prev_script, build BIP143 preimage
    # Need prev_value as int, prev_script as hex bytes
    if tx and tx["is_segwit"]:
        # ensure prev_value and prev_script available
        ...
        preimage = build_bip143_preimage(tx, input_index, script_bytes, value_sats, sighash_int)
        hz = dblsha(preimage)
    ...
    if tx:
        ...
        if prev_script is None or prev_script == "":
            # try to obtain prev_script from tx.inputs' scriptSig? for P2PKH scriptPubKey known from prev tx
            if tx["inputs"][input_index]["scriptSig"]:
                # not reliable: scriptSig contains signature+pubkey, not scriptPubKey
                pass
...
# ---------- main ----------
def main():
    p = argparse.ArgumentParser(description="Compute z (message hash) for ECDSA signatures from der-summary CSV")
